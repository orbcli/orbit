...
**Toolchain as a workspace member** — The auxiliary infrastructure you maintain by hand — agent skills, CI/CD, linters, compilers, PRD/notes — rarely lives in the repo you're editing, so its feedback has nowhere to accumulate. Add a dedicated knowledge repo to the workspace and the agent routes pitfalls and fixes into it via branch + PR. A solo dev sweeps it on their own cadence; a team's toolchain owner aggregates daily — same git-native mechanism, no server. See [`docs/recipes.md`](docs/recipes.md#accumulate-toolchain-feedback-in-a-knowledge-repo).
