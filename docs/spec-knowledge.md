...
- At natural breakpoints (wrap-up before done, or the jot buffer warning), the agent aggregates: `orbit jot <repo> --pop` → `orbit info <repo>` → merge entries into memo → `orbit memo <repo>`
...
2. **Trigger (reliable, not memory-based).** Independent triggers resurface pending captures so aggregation is not left to the agent remembering:
   - *Building / overflow* — `orbit jot` checks the repo's queue against `jot.bufferSize` (default `memo.minLines` = 4): above half it prints a `building` count; past the buffer it prints an `overflow` warning naming the aggregation action (`jot <repo> --pop`, then merge).
   - *Cruise block* — bare `orbit context` and the `--startup`/reignite block show the same per-repo status as a snapshot (`N jots (building|overflow)`), so a compaction or session boundary cannot hide a filling queue.
   - *Wrap-up / done* — `orbit done` warns per repo for any un-popped jot entries (any count, however small), also printing the `card budget is <min>~<max> lines` reminder when jots remain or a card is over budget.
3. **Aggregate (memo, at checkpoint, owner-only).** `orbit jot <repo> --pop` drains the queue; the owner folds entries into the card (read-modify-write) and writes back via `orbit memo <repo>`. Curation stays serial and owner-scoped (concurrent writers lose updates). When the card passes the ceiling, curate rather than append.
...
### Memo thin / over-budget surfacing (inline computation, no sentinel)

The weakest link in the path above is the no/low-memo repo an agent adds, works in, never jots, and finishes — leaving the pool repo with no reusable context, and compaction erasing any in-context "write the memo later" intent. Orbit closes it **without any durable sentinel**: the condition (thin memo, over-budget memo, leftover jots) is recomputed inline from the memo file and the jot queue on every read, so it can never be lost — only ignored. The same per-repo status surfaces redundantly:

- **Add-time stderr (CLI).** When `orbit add` sees a thin/missing memo, it prints a one-shot stderr naming the `explore.paths` scope — explore and write a card before done. This fires at the high-attention moment and is the carrier of the bounded exploration scope.
- **Cruise block (hooks + CLI).** Bare `orbit context` and the `--startup`/reignite block show `memo thin` / `memo over budget` in the per-repo status, so the state resurfaces after compaction and at every session start.
- **CLI gate.** `orbit done` warns per repo: leftover jots (`pop + merge`), thin memo with no capture (`explore + write`), over-budget card (`curate once`). It never blocks — done is the final backstop.

A thin memo is *absence* (explore-and-write); an over-budget card is *surplus* (curate-and-cut) — opposite fixes, one shared surfacing path. Over-budget handling is best-effort: `orbit memo` writeback prints a one-shot stderr when the card passes `memo.maxLines + memo.minLines` (the buffer means best-effort curation landing slightly over the ceiling is left alone), and the done gate recommends one curation pass per episode — orbit does not loop.

This surfacing model is one instance of a broader mechanism: **warnings are orbit's guidance layer** (PRINCIPLES.md Principle 8). Orbit has no enforcement runtime, so it steers the agent through stderr hints — card budget, jot overflow, staleness/sync-behind, tracking notes — that the agent is expected to act on. Warnings *guide*; they do not block (the sole exceptions are explicit gates like `orbit done`'s), and they never pollute stdout, which stays machine-readable data. The complete catalogue and message-format contract live in [spec-warnings.md](spec-warnings.md).
