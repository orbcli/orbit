...
| `N branches kept — review: …` + `after confirming …: orbit prune <ws> --force` | closing block of a `prune` run that skipped unmerged branches (live or ghost residue); one suggestion per workspace; scoped skips only — a raw skip belongs to the raw report below | `orbit_prune` |
| `untraceable branches (raw, no remote, no workspace) — human disposal:` + per-branch status/review lines + `after confirming …, force-delete:` header + `branch -D` commands | `orbit prune` enumeration report of branches traceable to nothing; orbit never deletes them | `orbit_prune_raw_residue` |
...
No **skip/refusal line** mentions `--force`, even where it would in fact proceed: the flag exists for a user who decides to discard work, not as an escape hatch the tool suggests mid-flow. The closing block is the deliberate exception — it comes after the full report, addresses the root operator, and leads with the review command plus the confirm-useless caveat. All applicable data-guard reasons are reported **together** in one line rather than one per run — `--force` releases them as a single decision, so the operator has to see the whole set to make that decision once. Under `--dry-run` the skips are reported on stdout as `would skip: <ws> (<reasons>)`, same wording rule. The guard contract itself lives in [spec-lifecycle.md](spec-lifecycle.md#prune-safety-guards).
